defmodule Mix.Tasks.MagicAuth.Install do
  ...
  defp inject_js_magic_auth_hook() do
    with {:ok, js_file_content} <- File.read(js_file()),
         ...
         {:ok, js_file_content} <- add_magic_auth_hooks(js_file_content) do
      File.write!(js_file(), js_file_content)
    else
      :already_injected ->
        :ok
  ...
  @hooks_object ~r/hooks:\s*\{([^{}]*)\}/

  defp add_magic_auth_hooks(js_file_content) do
    cond do
      String.contains?(js_file_content, "hooks:") ->
        add_to_existing_hooks(js_file_content)

      String.contains?(js_file_content, "params: {_csrf_token: csrfToken}\n") ->
        {:ok,
         String.replace(
           js_file_content,
  ...
  # Apps generated by Phoenix 1.8 already declare hooks (e.g. `hooks: {...colocatedHooks}`),
  # so MagicAuthHooks is appended to them. Anything other than a single object literal, such
  # as `hooks: Hooks`, is left for the user to change manually.
  defp add_to_existing_hooks(js_file_content) do
    case Regex.scan(@hooks_object, js_file_content) do
      [[_hooks_object, _hooks]] ->
        {:ok,
         Regex.replace(@hooks_object, js_file_content, fn _hooks_object, hooks ->
           "hooks: {" <> append_magic_auth_hooks(hooks) <> "}"
         end)}
  ...
  defp append_magic_auth_hooks(hooks) do
    trimmed_hooks = String.trim_trailing(hooks)

    case String.trim_trailing(trimmed_hooks, ",") do
      "" -> "...MagicAuthHooks"
      existing_hooks -> existing_hooks <> ", ...MagicAuthHooks" <> String.replace_prefix(hooks, trimmed_hooks, "")
    end
  end
