...
### Fixed
  ...
  - `mix magic_auth.install` now adds `MagicAuthHooks` to the hooks already declared in `app.js`, such as the
    `hooks: {...colocatedHooks}` generated by Phoenix 1.8, instead of asking for it to be added manually.

### Documentation

  - Added instructions for disabling rate limiting in the test environment to the [Testing Guide](guides/testing.md).
  - Updated the installation snippet in the Getting Started guide to the current version.
